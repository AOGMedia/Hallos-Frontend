...
interface UploadedShortCardProps {
  short: VideoListItem;
}

export function UploadedShortCard({ short }: UploadedShortCardProps) {
  // Mock data for demonstration
  const description = 'Description appears here\n...and wraps here';
  const postedDate = new Date(Date.now() - 28 * 24 * 60 * 60 * 1000);
  const rating = 0;
  const ratingCount = 0;

  return (
    <Link
      href={`/dashboard/video/${short.id}`}
    ...
    >
      ...
      <div className="rounded-[15px] border border-[rgba(234,234,234,0.10)] bg-[rgba(242,242,242,0.04)] overflow-hidden flex flex-col gap-[15.09px]">
        ...
        <div className="relative w-full aspect-video">
          <Image
            src={short.thumbnailUrl || '/images/video-placeholder.svg'}
            alt={short.title}
            fill
            ...
            <div className="flex items-center gap-[15.09px]">
              ...
              <div className="flex items-center gap-[5.66px]">
                ...
                <span className="text-xs font-normal leading-3 text-[#f2f2f2]">
                  {formatViewCount(short.viewsCount || 0)} views
                </span>
              </div>
